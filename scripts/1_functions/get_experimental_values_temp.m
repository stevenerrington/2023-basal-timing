...
% % ///////////////////////////////////////////// %
for trl_i=1:length(PDS.timesoffreeoutcomes_first)
    % Find time of spikes relevant to cue onset
    spk=PDS(1).sptimes{trl_i}-PDS(1).timetargeton(trl_i);
    % Change timing to ms, rather than sec
    spk=(spk*1000)+event_zero-1;
...
SDFFREE = plot_mean_psth({RastersFree},params.sdf.gauss_ms,1,size(RastersFree,2),1); %make spike density functions for displaying and average across neurons for displaying
% Cut the SDF to the relevant epoch of interest
SDFFREE = SDFFREE(:,event_zero+params.sdf.window(1):event_zero+params.sdf.window(1));
