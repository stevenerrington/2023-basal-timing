...
for neuron_i = 1:size(bf_data_CS,1)
    ...
    for epoch_i = 1:length(epoch_labels)
       ...
       fano_continuous = find(ismember(bf_data_CS.fano(neuron_i).time,...
        ...
        fano_prob100(neuron_i,epoch_i) = nanmean(bf_data_CS.fano(neuron_i).raw.prob100(fano_continuous));
        fano_prob75(neuron_i,epoch_i) = nanmean(bf_data_CS.fano(neuron_i).raw.prob75(fano_continuous));
        fano_prob50(neuron_i,epoch_i) = nanmean(bf_data_CS.fano(neuron_i).raw.prob50(fano_continuous));
        fano_prob25(neuron_i,epoch_i) = nanmean(bf_data_CS.fano(neuron_i).raw.prob25(fano_continuous));
        fano_prob0(neuron_i,epoch_i) = nanmean(bf_data_CS.fano(neuron_i).raw.prob0(fano_continuous));
    end
...
% CS (NIH) data:
fano_100_ramping = nanmean(fano_prob100(bf_datasheet_CS.cluster_id == 2,:));
fano_75_ramping = nanmean(fano_prob75(bf_datasheet_CS.cluster_id == 2,:));
fano_50_ramping = nanmean(fano_prob50(bf_datasheet_CS.cluster_id == 2,:));
fano_25_ramping = nanmean(fano_prob25(bf_datasheet_CS.cluster_id == 2,:));
fano_0_ramping = nanmean(fano_prob0(bf_datasheet_CS.cluster_id == 2,:));
fano_all_ramping = [fano_100_ramping; fano_75_ramping; fano_50_ramping; fano_25_ramping; fano_0_ramping];

% CS (WUSTL) data:
fano_100_phasic = nanmean(fano_prob100(bf_datasheet_CS.cluster_id == 1,:));
fano_75_phasic = nanmean(fano_prob75(bf_datasheet_CS.cluster_id == 1,:));
fano_50_phasic = nanmean(fano_prob50(bf_datasheet_CS.cluster_id == 1 ,:));
fano_25_phasic = nanmean(fano_prob25(bf_datasheet_CS.cluster_id == 1,:));
fano_0_phasic = nanmean(fano_prob0(bf_datasheet_CS.cluster_id == 1,:));
