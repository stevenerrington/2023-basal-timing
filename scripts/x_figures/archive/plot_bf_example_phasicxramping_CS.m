function plot_bf_example_phasicxramping_CS(dirs,data_in)
% Input variables
...
figure_plot(3,1).axe_property('XLim',xlim_input,'YLim',[0 6]);
...
figure_plot(1,2).axe_property('XLim',xlim_input);
figure_plot(1,2).set_names('x','Time from CS Onset (ms)','y','Trial');
...
figure_plot(2,2).axe_property('XLim',xlim_input,'YLim',ylim_input);
figure_plot(2,2).set_names('x','Time from CS Onset (ms)','y','Firing rate (spk/sec)');
...
figure_plot(3,2).axe_property('XLim',xlim_input,'YLim',[0 6]);
figure_plot(3,2).set_names('x','Time from CS Onset (ms)','y','Fano Factor');
figure_plot(3,2).set_color_options('map',color_scheme);
figure_plot(3,2).geom_vline('xintercept',0,'style','k-');
figure_plot(3,2).geom_vline('xintercept',1500,'style','k-');
...
filename = fullfile(dirs.root,'results','raster_sdf_fano_cstask_figure_ramping_phasic.pdf');
