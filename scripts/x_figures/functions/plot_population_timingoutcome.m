function figure_plot_out = plot_population_timingoutcome(data_in,plot_trial_types,params)
...
epoch.postOutcome = [0 200]; epoch_zero.postOutcome = [1500 1500];
...
for neuron_i = 1:size(data_in,1)
    ...
    for trial_type_i = 1:length(plot_trial_types)
        ...
        switch trial_type_label
            case {'p25s_75l_short' , 'p25s_75l_long'}; plot_group = [plot_group; {'1_25%'}];
            case {'p50s_50l_short' , 'p50s_50l_long'}; plot_group = [plot_group; {'2_50%'}];
            case {'p75s_25l_short' , 'p75s_25l_long'}; plot_group = [plot_group; {'3_75%'}];
        end
        
        if endsWith( trial_type_label , 'long' )
...
figure_plot(2,1).facet_grid([],plot_group);
