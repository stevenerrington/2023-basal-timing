...
% CDF latency
figure_data(5,1) = gramm('x',{cdf_latency_bf.data.prob25nd(:,1);cdf_latency_bf.data.prob50nd(:,1);cdf_latency_bf.data.prob75nd(:,1)},...
    'y',{cdf_latency_bf.data.prob25nd(:,2)';cdf_latency_bf.data.prob50nd(:,2)';cdf_latency_bf.data.prob75nd(:,2)'},...
    'color',plot_trial_types);
figure_data(5,1).geom_line;
figure_data(5,1).set_names('x','Time from Outcome (ms)','y','CDF');
figure_data(5,1).set_color_options('map',bf_color([2,3,4],:));
figure_data(5,1).no_legend;

figure_data(6,1) = gramm('x',{cdf_latency_bg.data.prob25nd(:,1);cdf_latency_bg.data.prob50nd(:,1);cdf_latency_bg.data.prob75nd(:,1)},...
    'y',{cdf_latency_bg.data.prob25nd(:,2)';cdf_latency_bg.data.prob50nd(:,2)';cdf_latency_bg.data.prob75nd(:,2)'},...
    'color',plot_trial_types);
figure_data(6,1).geom_line;
figure_data(6,1).set_names('x','Time from Outcome (ms)','y','CDF');
figure_data(6,1).set_color_options('map',striatum_color([2,3,4],:));
figure_data(6,1).no_legend;
...
figure_data(7,1)=gramm('x',cond_label_bf,...
    'y',[expo_fit_data_bf.average.prob25nd;expo_fit_data_bf.average.prob50nd;expo_fit_data_bf.average.prob75nd],...
    'color',cond_label_bf);
figure_data(7,1).stat_summary('geom',{'bar','errorbar'},'width',1,'dodge',1);
figure_data(7,1).no_legend;
figure_data(7,1).set_color_options('map',bf_color([2,3,4],:));
figure_data(7,1).axe_property('YLim',[0 0.02]);
figure_data(7,1).set_names('y','Lambda');

figure_data(8,1)=gramm('x',cond_label_bg,...
    'y',[expo_fit_data_bg.average.prob25nd;expo_fit_data_bg.average.prob50nd;expo_fit_data_bg.average.prob75nd],...
    'color',cond_label_bg);
figure_data(8,1).stat_summary('geom',{'bar','errorbar'},'width',1,'dodge',1);
figure_data(8,1).no_legend;
figure_data(8,1).set_color_options('map',striatum_color([2,3,4],:));
figure_data(8,1).axe_property('YLim',[0 0.04]);
figure_data(8,1).set_names('y','Lambda');

%% Define figure properties
figure_data(1,1).axe_property('XColor',[1 1 1]);
figure_data(2,1).axe_property('XTicks',[0 250 400 600]);
figure_data(3,1).axe_property('XColor',[1 1 1],'XTicks',[]);
figure_data(4,1).axe_property('YLim',[0 2],'XColor',[1 1 1],'XTicks',[]);
figure_data(5,1).axe_property('XLim',[50 400],'YLim',[0 1]);
figure_data(6,1).axe_property('XLim',[50 400],'YLim',[0 1]);

% BF SDF
figure_data(1,1).set_layout_options...
    ('Position',[0.1 0.6 0.2 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
...
figure_data(2,1).set_layout_options...
    ('Position',[0.1 0.15 0.2 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
    'legend',false,... % No need to display legend for side histograms
    'margin_height',[0.02 0.05],... %We set custom margins, values must be coordinated between the different elements so that alignment is maintained
    'margin_width',[0.1 0.02],...
    'redraw',false);
...
figure_data(3,1).set_layout_options...
    ('Position',[0.35 0.6 0.15 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
    'legend',false,... % No need to display legend for side histograms
    'margin_height',[0.02 0.05],... %We set custom margins, values must be coordinated between the different elements so that alignment is maintained
    'margin_width',[0.1 0.02],...
    'redraw',false);
...
figure_data(4,1).set_layout_options...
    ('Position',[0.35 0.15 0.15 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
    'legend',false,... % No need to display legend for side histograms
    'margin_height',[0.02 0.05],... %We set custom margins, values must be coordinated between the different elements so that alignment is maintained
    'margin_width',[0.1 0.02],...
    'redraw',false);
...
figure_data(5,1).set_layout_options...
    ('Position',[0.55 0.6 0.15 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
    'legend',false,... % No need to display legend for side histograms
    'margin_height',[0.02 0.05],... %We set custom margins, values must be coordinated between the different elements so that alignment is maintained
    'margin_width',[0.1 0.02],...
    'redraw',false);
...
figure_data(6,1).set_layout_options...
    ('Position',[0.55 0.15 0.15 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
    'legend',false,... % No need to display legend for side histograms
    'margin_height',[0.02 0.05],... %We set custom margins, values must be coordinated between the different elements so that alignment is maintained
    'margin_width',[0.1 0.02],...
    'redraw',false);
...
figure_data(7,1).set_layout_options...
    ('Position',[0.75 0.6 0.15 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
    'legend',false,... % No need to display legend for side histograms
    'margin_height',[0.02 0.05],... %We set custom margins, values must be coordinated between the different elements so that alignment is maintained
    'margin_width',[0.1 0.02],...
    'redraw',false);
...
figure_data(8,1).set_layout_options...
    ('Position',[0.75 0.15 0.15 0.3],... %Set the position in the figure (as in standard 'Position' axe property)
    'legend',false,... % No need to display legend for side histograms
    'margin_height',[0.02 0.05],... %We set custom margins, values must be coordinated between the different elements so that alignment is maintained
    'margin_width',[0.1 0.02],...
    'redraw',false);
