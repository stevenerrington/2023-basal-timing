function bfstri_pca_explore_trace(bf_data_traceExp,striatum_data_traceExp)
%% Example extraction code
% Tidy workspace
clear timewin

% Define trials/sdf normalization
trial_type_in = 'notrace_uncertain'; % Corresponds to a trial type in the data_in structure
...
for area_in = {'bf','stri'}
    ...
    timewin = [];
    timewin = [0:5:outcome_time+1000]; % Denotes the time window to extract the SDF from (rel to CS onset)
    ...
    for neuron_i = 1:size(data_in,1)
        ...
        sdf_50_max(neuron_i,:) =...
            ...
            max(nanmean(data_in.sdf{neuron_i}(data_in.trials{neuron_i}.(trial_type_in),timewin+time_zero_sdf)));
    ...
    title([area_in{1} ' - ' sdf_norm ' - ' trial_type_in], 'Interpreter', 'none')
