...
% Extract parameters
fill_missing_data = params.saccade.fill_missing_data;
smoothen = params.saccade.smoothen;
sampling_rate = params.saccade.sampling_rate;
fix_vel_thres = params.saccade.fix_vel_thres;
lambda = params.saccade.lambda;
combine_intv_thres = params.saccade.combine_intv_thres;
saccade_dur_thres = params.saccade.saccade_dur_thres;
saccade_amp_thres = params.saccade.saccade_amp_thres;
...
% inputs£º8 variables as parameters 
% #1 eyetrace:a 2-column array, [Xpos, Ypos] (recommend unit: degree)
% #2 fill_missing_data: mostly for eye blinks, 
%    a bool value: True/False (recommend: False. True for matlab2018 and later versions); 
%    defalut method: linear insertion (you can change the method in this script)
% #2 smoothen: for smoothening eyetrace, Bool value: True/False
% #3 sampling_rate (recommend:1000Hz)
% #4 fixation velocity maximum threshold (recommend:30 degree/s)
% #5 lambda (recommend:7.5)
% #6 combine_intv_thres (recommend:20 ms)
% #7 saccade_dur_thres (recommend:5 ms)
% #8 saccade_amp_thres 
...
%% check data input 
if size(eyetrace,1) < 10
    disp('>>>>ERROR: input eyetrace is too short (less than 10 sampling points).')
else
    %%  fill missing data
    if fill_missing_data == true && sum(sum(isnan(eyetrace))) ~= 0
    %     disp(">>>> WARNNING: There are missing data points in eyetrace.")
    %     disp(">>>> WARNNING: Fill missing data by 'linear' insertion.")
        eyetrace = fillmissing(eyetrace,'linear');
    end
    %% smoothen data 
    if smoothen == true
    %   disp(">>>> WARNNING: Smoothen the data.") 
       eyetrace = [smooth(eyetrace(:,1),'moving'),...
           smooth(eyetrace(:,2),'moving')];
    end
    ...
    if ~isnan(over_threshold_ind) % found potential saccade 
    ...
    else % didn't find any potential saccade 
        saccade_info = [];
    end
end
end
