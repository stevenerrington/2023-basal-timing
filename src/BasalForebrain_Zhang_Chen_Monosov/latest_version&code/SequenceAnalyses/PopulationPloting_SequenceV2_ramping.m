...
%%%%%% novelty ramping
regressionrange = [200:900]';
%Timinglongd = vertcat(DataTP(:).all_long_delivered);
fitdata = mean(F1(:, regressionrange))';
base = [ones(length(regressionrange),1), regressionrange];
[b,bint] = regress(fitdata,base,0.01)
plot(regressionrange, b(1)+b(2)*regressionrange,'Color', 'r','lineWidth', 2);

ydata = F1(:, regressionrange)';
xdata = repmat(regressionrange,1,size(ydata,2));
[p,rho] = permutation_pair_test_fast(xdata(:),ydata(:), PermNum,'rankcorr');
if bint(2,1)*bint(2,2)>0
    text(500,0.3,['p=',num2str(p,2)]);
    text(500,0.4,['rho=', num2str(rho,2)]);
end

%%%%%%% reward ramping
regressionrange = [3200:3900]';
%Timinglongd = vertcat(DataTP(:).all_long_delivered);
fitdata = mean(F1(:, regressionrange))';
base = [ones(length(regressionrange),1), regressionrange];
[b,bint] = regress(fitdata,base,0.01)
...
[p,rho] = permutation_pair_test_fast(xdata(:),ydata(:), PermNum,'rankcorr');
if bint(2,1)*bint(2,2)>0
    text(3500,0.4,['p=',num2str(p,2)]);
    text(3500,0.5,['rho=', num2str(rho,2)]);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
nsubplot(450,450,1:450,1:450)
set(gca,'ticklength',4*get(gca,'ticklength'))
NovUseful=vertcat(savestructTonic(1,:).NovUseful);
NovNotUseful=vertcat(savestructTonic(1,:).NovNotUseful);
NovUsefulFam=vertcat(savestructTonic(1,:).NovUsefulFam);
NovNotUsefulFam=vertcat(savestructTonic(1,:).NovNotUsefulFam);
...
FamSS=[];
NovSS=[];
for x=1:length(savestructTonic)
    if isempty(savestructTonic(x).NovUseful_)==0 & isempty(savestructTonic(x).NovNotUseful_)==0
        
        NovUseful=[NovUseful;savestructTonic(x).NovUseful_]
        NovNotUseful=[NovNotUseful;savestructTonic(x).NovNotUseful_]
        FamSS=[FamSS;savestructTonic(x).FamSS_];
        NovSS=[NovSS;savestructTonic(x).NovSS_];
    end
end
...
if FamIncludeFirst==1
    FamSS=vertcat(savestructTonic(1,:).FamSS)
else
    FamSS=vertcat(savestructTonic(1,:).FamSSa)
end
NovSS=vertcat(savestructTonic(1,:).NovSS)
...
set(gca,'ticklength',4*get(gca,'ticklength'))
ROC=vertcat(savestructTonic(1,:).Nov_r);
ROCp=vertcat(savestructTonic(1,:).Nov_p);
...
F=vertcat(savestructPhasic(1,:).NovNotUseful_);
p=signrank(N,F)
t=text(15,10,['p ='   mat2str(p)]); set(t, 'FontSize', 8);
...
scatter(N,F, 10, 'r', 'filled');
N1=vertcat(savestructPhasic(1,:).BEfiring);
F1=vertcat(savestructPhasic(1,:).NOBEfiring);
scatter(N1,F1, 10, 'k', 'filled');
axis([-5 25 -5 25])
line([-5 25],  [-5 25])
axis square;
p=signrank(N,F)
t=text(15,5,['p tonic='   mat2str(p)]); set(t, 'FontSize', 8);
p=signrank(N1,F1)
t=text(15,15,['p phasic='   mat2str(p)]); set(t, 'FontSize', 8);
