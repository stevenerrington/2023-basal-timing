...
Group1Value=[];
Group1Unc=[];
for x=1:length(Group1)
    X(1:500)=0;
    X(find(Group1(x).CorvalP(1:500)<TTT))=1;
    Group1Value=[Group1Value; X]; clear X 
    
     X(1:500)=0;
    X(find(Group1(x).CoruncP(1:500)<TTT))=1;
    Group1Unc=[Group1Unc; X]; clear X x
end

Group2Value=[];
Group2Unc=[];
for x=1:length(Group2)
    X(1:500)=0;
    X(find(Group2(x).CorvalP(1:500)<TTT))=1;
    Group2Value=[Group2Value; X]; clear X
    
    X(1:500)=0;
    X(find(Group2(x).CoruncP(1:500)<TTT))=1;
    Group2Unc=[Group2Unc; X]; clear X x
end

Group1Value=nansum(Group1Value)./length(Group1)
Group1Unc=nansum(Group1Unc)./length(Group1)
Group2Value=nansum(Group2Value)./length(Group2)
Group2Unc=nansum(Group2Unc)./length(Group2)
...
% imagesc([Group1Value; Group1Unc])
% caxis([0 ThreshForsigPer])
% colormap('bone');
%
...
% %nsubplot(400,400,101:175,61:110); 
...
% %nsubplot(400,400,101:175,121:170); 
...
% %nsubplot(400,400,101:175,181:230); 
...
% %nsubplot(400,400,201:275,181:230); 
...
%%nsubplot(400,400,201:275,281:330); 
...
%nsubplot(400,400,1:50,1:50); 

figure; hold on; set(gca,'ticklength',4*get(gca,'ticklength'))
plot(nanmean(vertcat(Group1(1,:).actp100R)),'Color',[0 0 0],'LineWidth',1)
xlim([0 1000])
ylim([YYLIMitm YYLIMit])
title('100')

%nsubplot(400,400,1:50,71:120); 
figure; hold on; set(gca,'ticklength',4*get(gca,'ticklength'))
