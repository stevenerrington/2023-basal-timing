...
for bnbn=1:length(alltrialsid)
    try
        ...
        %look at tS
        st_=evtime.cueontime(bnbn);%-d(Tsac).aStartTime;
        st_=st_-evtime.analogstart(bnbn);
        en_=st_+1000;
        lickstbase=d(bnbn).Signals(6).Signal(st_:en_);
        
        lickstbase=filtfilt(bb,aa,double(lickstbase));
        lickst=filtfilt(bb,aa,double(lickst));
        
        LickCS=[LickCS;lickst];
        LickBase=[LickBase; lickstbase];
...
numberofstd=2;
baseline=LickBase(completedtrial,:);
baseline=baseline(:);
