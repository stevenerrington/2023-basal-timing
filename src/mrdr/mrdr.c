	...
	for(i = 0; i < nrhs; ++i) {
		...
		if(arg[0] == '-') {
			...
			case 'a':
				mrdr_absTime = -1;
				/*
				  mexWarnMsgTxt("using absolute time\n");
				*/
				break;
			case 'd':
				++i;
...
void readData(int32_t nNewTrials, int32_t nPrevTrials, mxArray *plhs)
{
	...
	for(i = 0; i < nNewTrials; ++i) {
		...
		if((ri = rexGetTrial(trialNumber, 1)) != (RexInfo *)NULL) {
			...
			if(ri->nEvents > 0) {
                ptr = cvt_events(ri->nEvents, tOffset, ri->events);
			...
			if(ri->nUnits > 0) {
                ptr = cvt_units(ri->nUnits, tOffset, ri->unitCodes, ri->rexunits);
