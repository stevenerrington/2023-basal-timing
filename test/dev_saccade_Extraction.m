...
for trial_type_i = 1:length(trial_type_in)
    ...
    sacc_raster = zeros(length(trials_in),length(params.eye.alignWin));
    ...
    data_sacc_times{trial_type_i} = sacc_times_out;
    data_sacc_labels{trial_type_i} = sacc_labels_out;
end

%% Concatenate
plot_sacc_times = []; plot_sacc_labels = [];

for trial_type_i = 1:length(trial_type_in)
    plot_sacc_times = [plot_sacc_times; data_sacc_times{trial_type_i}];
    plot_sacc_labels = [plot_sacc_labels; data_sacc_labels{trial_type_i}];
end
...
%% Raster saccades
clear figure_plot
figure_plot(1,1)=gramm('x',plot_sacc_times,'color',plot_sacc_labels);
...
figure_plot(2,1)=gramm('x',plot_sacc_times,'color',plot_sacc_labels);
