function [figure_plot_out, figure_plot, plot_data] = plot_example_punish(data_in,plot_trial_types,params,fig_flag)
...
for trial_type_i = 1:length(plot_trial_types)
    ...
    trials_in = []; trials_in = data_in.trials.(trial_type_label);
    n_trls = size(trials_in,2);
    
    plot_sdf_data = [plot_sdf_data ; num2cell(data_in.sdf(trials_in,:),2)];
    plot_label = [plot_label; repmat({[int2str(trial_type_i) '_' (trial_type_label)]},n_trls,1)];
    
    spkTimes = {};
    for trl_i = 1:n_trls
        spkTimes{trl_i,1} = find(data_in.rasters(trials_in(trl_i),:) == 1) - time_zero;
    end
    
    plot_spk_data = [plot_spk_data; spkTimes];
    
    
    plot_fano_data = [plot_fano_data; {data_in.fano.smooth.(trial_type_label)}];
...
if fig_flag == 1
    ...
    % Fano factor
    figure_plot(3,1)=gramm('x',data_in.fano.time,'y',plot_fano_data,'color',plot_fano_label);
