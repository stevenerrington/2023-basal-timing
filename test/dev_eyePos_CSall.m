...
% session
for neuron_i = 1:size(p_gaze_data_all,1)
    y_fit = []; y_fit = polyfit([1:5],p_gaze_data_all(neuron_i,:),2);
    y_fit_data{neuron_i,1} = polyval(y_fit,[1:0.1:5]);
    y_fit_out(neuron_i,:) = y_fit;
end

for neuron_i = 1:size(p_gaze_data_1500,1)
    y_fit_1500 = []; y_fit_1500 = polyfit([1:5],p_gaze_data_1500(neuron_i,[1:5]),2);
    y_fit_data_1500{neuron_i,1} = polyval(y_fit_1500,[1:0.1:5]);
    y_fit_out_1500(neuron_i,:) = y_fit_1500;
end

for neuron_i = 1:size(p_gaze_data_2500,1)
    y_fit_2500 = []; y_fit_2500 = polyfit([1:5],p_gaze_data_2500(neuron_i,:),2);
    y_fit_data_2500{neuron_i,1} = polyval(y_fit_2500,[1:0.1:5]);
    y_fit_out_2500(neuron_i,:) = y_fit_2500;
end
