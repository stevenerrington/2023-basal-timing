...
%         regularPdsData = interp1(  relatveTimePDS , temp(:,1) , regularTimeVectorForPdsInterval  );
        ...
        %
           ...
           Pupil=[Pupil; nan(1,12000)];
           Xeye=[Xeye; nan(1,12000)];
           Yeye=[Yeye; nan(1,12000)]; 
   ...
   amp=10;
        ...
        temp(timelook)=1;
        try
            temp=temp(fix(PDS.timetargeton(x)*1000):fix(PDS.timetargeton(x)*1000)+3500); % change 3000 to 3300
        catch
            clear temp;
            temp(1:3501)=NaN;
        end
...
%         end
    ...
    for x=1:length(PDS.fractals)
        ...
        targrange=[targon_' : targon_'+3500]; %change 3250 to 3500
...
if contains(s.RecHemisphere{DATA_N}, 'right')
    d1 = 'L';
    d2 = 'R';
else
    d1 = 'R';
...
%wholeerrorrate = sum(temp_er)/numel(temp_er)
