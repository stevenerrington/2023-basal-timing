...
trial_type_in = 'fractal6105_2500'; % Corresponds to a trial type in the data_in structure
sdf_norm = 'zscore'; % zscore or max
area_list = {'striatum','bf'}; % striatum, bf_nih, bf_wustl
...
for area_i = 1:length(area_list)
    ...
    switch area_in
        case 'striatum'
            data_in = []; data_in = striatum_data_timingTask;
            outcome_time = 2500;
        case 'bf'
    ...
    params.pca.timewin = [0 1000]; params.pca.step = 1;
    ...
    sdf_data_in(isnan(sdf_data_in(:,1)),:) = [];
